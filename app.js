...
function attachHoldToAdd(card, item) {
  ...
  validItems.forEach((item, index) => {
    ...
    lines.push(`   ${t("wa_price_label")} ${formatPrice(item.price)}`);
    lines.push("");
  });
...
function checkoutWhatsApp() {
  ...
  const url = `https://wa.me/${CONFIG.WA_NUMBER}?text=${message}`;
...
function checkoutTelegram() {
  ...
  const url = `https://t.me/${CONFIG.TELEGRAM_USERNAME}?text=${message}`;
  ...
  function setupEvents() {
    ...
    const checkoutTelegramBtn =
      $("checkout-telegram-btn");
...
if (checkout) {
    ...
    if (checkoutTelegramBtn) {
      ...
      checkoutTelegramBtn.addEventListener(
        "click",
        checkoutTelegram
      );
